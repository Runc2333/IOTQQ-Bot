...
function init() {
	...
	config.registerSuperCommand("timebot", "timebot.js", "timebot", "开启/关闭TimeBot.仅管理员可用.\n[state]: (enable|disable) => (开启|关闭)", "[state]");
	sendTimeSticker();
...
function timebot(packet) {
	if (user.isAdmin(packet.FromUin, packet.FromGroupUin)) {
		var command = packet.Content.match(/(?<=\/)[a-z]{1,10}\s[a-z]{1,10}/i);
		if (command !== null) {
		...
		switch (parameter) {
			case "enable":
				var ENABLE_TIMEBOT_GROUP = config.get("TIMEBOT", "ENABLE_TIMEBOT_GROUP");
				if (ENABLE_TIMEBOT_GROUP.indexOf(packet.FromGroupUin.toString()) === -1) {
